...
public class MeteoOnline {
    ...
    public Meteo getOnlineWeather(Position position){
	    ...
	    boolean worked = fio.getForecast(""+position.getLati(),""+position.getLongi());
        JsonObject response = fio.getCurrently();
        if (worked){
            return new Meteo(Float.parseFloat(response.get("temperature").toString()), Float.parseFloat(response.get("precipProbability").toString()), response.get("summary").toString());
        } else {
