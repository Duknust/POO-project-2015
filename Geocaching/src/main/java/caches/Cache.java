...
public abstract class Cache implements Serializable, Comparable<Cache> {
	...
	protected GregorianCalendar publishDate; // Date cache was published
	protected GregorianCalendar creationDate; // Date cache was created
	protected String cacheID; // Cache ID number
	protected boolean premiumOnly; // Cache Premium
	protected String description; // Cache description
	protected Status cacheState; // Cache Status
	protected String cacheTitle; // Cache name
	protected UserAbstract owner; // Who placed the cache
	protected int cacheSize; // Type of container
	protected float difficulty; // How difficult is it to find the cache
	protected Position position;
	protected String hint; // Hints to find the cache
	protected TreeSet<Log> cache_Logs; // Cache logs
	protected Reviewer reviewer = null; // Reviewer responsible
	protected Data data = null; // System Data

	// Constructors
	public Cache(GregorianCalendar publishDate, GregorianCalendar creationDate,
			String cacheID, boolean premiumOnly, String description,
			Status cacheState, String cacheTitle, UserAbstract owner,
			int cacheSize, float difficulty, Position position, String hint,
			TreeSet<Log> cache_Logs, Reviewer reviewer, Data data) {
	...
	// w/o ID
	public Cache(GregorianCalendar creationDate, String description,
			String cacheTitle, int cacheSize, float difficulty,
			Position position, String hint, TreeSet<Log> cache_Logs,
	...
	public Cache(GregorianCalendar creationDate, String description,
			String cacheTitle, int cacheSize, float difficulty,
			Position position, String hint, TreeSet<Log> cache_Logs, Data data) {
		this.creationDate = creationDate;
		this.description = description;
		this.cacheState = Status.UNPUBLISHED;
		this.cacheTitle = cacheTitle;
	...
	public Cache(GregorianCalendar creationDate, String description,
			String cacheTitle, Position position, UserAbstract owner, Data data) {
		this.creationDate = creationDate;
		this.owner = owner;
		this.cacheTitle = cacheTitle;
		this.description = description;
	...
	public Cache(GregorianCalendar creationDate, String description,
			String cacheTitle, int cacheSize, float difficulty,
			Position position, String hint, boolean premium,
			UserAbstract owner, Data data) {
		this.creationDate = creationDate;
		this.owner = owner;
		this.cacheTitle = cacheTitle;
		this.description = description;
	...
	public boolean logCache(UserAbstract user, Log log) {
		...
		case UNPUBLISHED:
			...
			if (user.getRole() == Role.REVIEWER) {
				/*
				 * if (this.reviewer == null) { return false; // No Reviewer No
				 * Log } else if (this.reviewer.equals(user) == false) { return
				 * false; // If this Reviewer is not assigned, No Log }
				 */
				if (log.getLogType() != Log.Log_Type.REVIEWER_NOTE) { // Check
																		// Log
																		// Type
					return false;
				}
				ac = new Activity(new GregorianCalendar(),
						Activity.Type.REV_NOTE, this, user, log);
			...
			} else if (user.getRole() == Role.USER) // User
			{
				...
				{
					if (log.getLogType() != Log.Log_Type.NOTE) { // Check Log
																	// Type
						return false;
					}
					ac = new Activity(new GregorianCalendar(),
							Activity.Type.NOTE, this, user, log);
				} else // Not the Owner
				{
		...
		case ARCHIVED:
			...
			if (log.getLogType() == Log.Log_Type.NOTE) { // Check Log Type
				ac = new Activity(new GregorianCalendar(), Activity.Type.NOTE,
						this, user, log);
		...
		case ENABLED:
			...
			case DNF:
				if (user.getRole() == Role.USER) {
					ac = new Activity(new GregorianCalendar(),
							Activity.Type.DIDNT_FIND_CACHE, this, user, log);
				} else {
					return false;
				}
				break;
			case FOUND_IT:
				if (user.getRole() == Role.USER) {
					ac = new Activity(new GregorianCalendar(),
							Activity.Type.FOUND_CACHE, this, user, log);
				} else {
					return false;
				}
				break;
		...
		if (log.getLogType() == Log_Type.FOUND_IT) {// If the User Found It
			((User) user).incTotalFound(); // Then increment by 1 the Total
											// Founds
		}
	...
	public String genID(int size) {
		return "GC"
				+ Long.toHexString(Double.doubleToLongBits(Math.random()))
						.substring(15 - size, 15).toUpperCase();
	}
	...
	public boolean hasFound(User user) {
		for (Log l : this.cache_Logs) {
			if (l.getUser().equals(user)
					&& l.getLogType() == Log.Log_Type.FOUND_IT) { // If User has
																	// a 'Found
																	// It' Log
	...
	public Log_Type getFoundStatus(User user) { // Return if the user has Found
												// It or DNF it for cache
												// display
		Log_Type type = null;
		for (Log l : this.cache_Logs) {
			if (l.getUser().equals(user)) {
	...
	@Override
	public String toString() {
		return "Cache{" + "publishDate=" + formatDateTime(publishDate)
				+ ", cacheID='" + cacheID + '\'' + ", description='"
				+ description + '\'' + ", cacheState=" + cacheState
				+ ", cacheTitle='" + cacheTitle + '\'' + ", premiumOnly='"
				+ premiumOnly + '\'' + ", owner=" + owner + ", cacheSize="
				+ cacheSize + ", difficulty=" + difficulty + ", position="
				+ position + ", hint='" + hint + '\'' + ", cache_Logs="
				+ cache_Logs + '}';
	}
	...
	public String toSimpleListing() {
		return cacheID + " '" + cacheTitle + '\'' + " (" + getType() + ")"
				+ " D " + difficulty + " / T " + position.getDifficulty()
